...
.. literalinclude:: ../docs_src/getting_started/gs_00.py
    :linenos:
    :language: python
    :lines: 5-9,34-
    :emphasize-lines: 1,4
...
On line 5, we are calling the init_app method of the app instance. You should call this method after loading the configuration to your app: it will register OpenAPI blueprints and generic error handlers. You can always opt-out of these with appropriate configuration values.
...
Finally, the two last lines are a convenient way to start the app by running the file directly.
...
Registering a view function means binding a python function to an URL. Whenever a request sent to your server matches the rule you defined, the function, called a view function, will be run.
...
Let's register a view function that takes parameters. On line 14, you will find the method decorator we saw in the previous section. But on line 15, the view function takes two parameters with type hints and default values. It then returns them without modifying them.
...
This view function is pretty useless besides inspecting the values the function actually receives when it is called and this is precisely what we will do.
...
We start by defining a Pydantic BaseModel for our response. This model will be used to validate the outbound data of our view function. We first import BaseModel and Field from pydantic on line 1 and 2. On line 23-26, we define a subclass of BaseModel named ``Item`` with three fields: ``name``, ``price`` and ``count``. The ``name`` field is a string, the ``price`` field is a float and the ``count`` field is an int with a default value of 1.

.. literalinclude:: ../docs_src/getting_started/gs_00.py
    :linenos:
    :language: python
    :lines: 2-
    :emphasize-lines: 1,2,23-26

We then pass the ``Item`` model as the ``response_model`` argument of the ``@app.get`` decorator on line 24. Once again our view function isn't very useful beyond demonstrating that our return value will be processed through the ``Item`` model and not simply returning the ``{"name": "Bottle", "price": 5}`` dictionnary, casting the price into a float, and adding a default value of 1 to the count field.
...
.. image:: _static/GettingStartedOpenAPIDocumentation.png
...
If you want to learn more, you can check out our :doc:`Tutorial </tutorial/index>` that takes a more in-depth approach of how to properly use these three features.
...
Code for this example can be found `here <https://github.com/jcbianic/flask-jeroboam/tree/main/docs_src/getting_started/gs_00.py>`_. Examples are tested in  `this file <https://github.com/jcbianic/flask-jeroboam/tree/main/tests/test_docs/test_getting_started.py>`_.
